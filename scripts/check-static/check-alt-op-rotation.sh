...
# pick_op_type_table[] in child.c must be either reachable via the
# dedicated alt-op rotation (alt_op_rotation[]) or explicitly listed
# in scripts/check-static/alt-op-rotation.denylist with a reason.
#
...
extract_table_ops() {
	...
	awk -v tbl="$table" '
		...
		# single line.  Both forms are used in child.c.
...
if [ -n "$MISSING" ]; then
	missing_count="$(printf '%s\n' "$MISSING" | wc -l)"
	{
		...
		printf '%s\n' "$MISSING" | sed 's/^/    /'
		echo "  fix: either add the symbol to alt_op_rotation[] in child.c, OR add a line to"
