...
# pick_op_type_table[] in child-altop.c must be either reachable via
...
extract_table_ops() {
	...
	awk -v tbl="$table" '
		...
		# single line.  Both forms are used in child-altop.c.
...
if [ -n "$MISSING" ]; then
	missing_count="$(printf '%s\n' "$MISSING" | wc -l)"
	{
		...
		printf '%s\n' "$MISSING" | sed 's/^/    /'
		echo "  fix: either add the symbol to alt_op_rotation[] in child-altop.c, OR add a line to"
