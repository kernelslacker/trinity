...
#   1. op_dispatch[] in child.c has exactly one designated initializer
#      per CHILD_OP_* enum value.
#   2. alt_op_name() in child.c has exactly one case label per
...
dispatch_count=$(awk '
	/static bool .*op_dispatch\[NR_CHILD_OP_TYPES\].*= \{/ { inside = 1; next }
	inside && /^\};/         { inside = 0 }
	inside && /^\s*\[CHILD_OP_[A-Z0-9_]+\]\s*=/ { print }
' "$CHILD_C" | wc -l)
...
if [ "$dispatch_count" -ne "$enum_count" ]; then
	fail "op_dispatch[] has $dispatch_count entries but enum has $enum_count values" \
		"see child.c op_dispatch[] and include/child.h enum child_op_type"
fi
...
if [ "$altname_count" -ne "$enum_count" ]; then
	fail "alt_op_name() covers $altname_count of $enum_count CHILD_OP_* values" \
		"see child.c alt_op_name() and include/child.h enum child_op_type"
fi
