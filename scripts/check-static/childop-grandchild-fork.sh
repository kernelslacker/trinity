...
#   UNPARSED:file:line:hash -- fork() site whose child branch the scanner could not
#                              locate (unrecognised pattern or window miss)
#
# Keyed baseline: scripts/check-static/childop-grandchild-fork.baseline
# Each entry is one "file:line:hash" or "UNPARSED:file:line:hash" string.  Any result
...
if [ "$MODE" = "regen" ]; then
	{
		...
		printf '# Each entry is "file:line:hash" (uncovered child branch) or\n'
		printf '# "UNPARSED:file:line:hash" (fork() site whose child branch the scanner\n'
...
if [ "${#new_findings[@]}" -gt 0 ]; then
	{
		...
		echo "  For 'file:line:hash' entries:"
		...
		echo "  For 'UNPARSED:file:line:hash' entries:"
		...
		echo "  Hash = first 8 hex chars of sha256 of the both-ends-trimmed line content:"
		echo "  sed -n Np <file> | sed 's/^[[:space:]]*//;s/[[:space:]]*\$//' | sha256sum | cut -c1-8"
