...
## --no-deferred-free-batch

Turns off per-iteration mprotect batching inside
`persist/deferred-free.c`, restoring the per-mutation tripwire.

Each protection region (`alloc_track` + hash, `ring_control`, `ring`)
can be flipped RW->write->RO/NONE by every metadata mutation:
`deferred_alloc_track`, `alloc_track_consume`, ring
enqueue/tick/evict, and every `rc_unlock` pair around `ring_count` /
`occupied_mask` updates.  That is the tightest possible corruption
tripwire and it is also expensive: mprotect was ~half of trinity's
child syscall traffic when it ran on every mutation.

Batching -- now the default -- collapses the N per-mutation brackets
within one iteration-phase into ~1 real flip per region per phase,
cutting the `*_rw_calls` / `*_ro_calls` counters by ~2x+ (measured
against the untraced-cost counters landed in `ded23739`).  The regions
are still read-only outside the phase window, so the tripwire still
catches a stray write from anywhere else in the program; what it no
longer pins down is which mutation inside a phase did it.

Pass this flag when a corruption reproducer needs that resolution.
