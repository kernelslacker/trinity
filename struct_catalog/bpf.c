 ...
 * members of union bpf_attr (enable_stats.*, iter_create.*,
 * token_create.*, ...), so offsetof and the schema field names
 * use dotted forms.  find_field_index_in walks the local fields[]
 * by strcmp on the dotted name; FT_LEN_BYTES.buf_field below uses
 * the same form so the pairing resolves.
 *
