...
/*
 * SHADOW cfactual control-replay harness (256·A tie-in: ONE reexec-
 * with-pin path shared with the arg-perturbation lane -- the follow-up
 * unification of perturb_replayed_struct_field's mutate-then-replay
 * layers onto this helper is a separate row).  Called from
 * cmp_hints_cfactual_capture() for each captured candidate to run the
 * causal A/B: re-dispatch the same syscall with fresh args EXCEPT the
 * caller-named slot pinned to a control value that is guaranteed to
 * differ from the parent-consumed hint's value.  The classifier reads
 * *found_new_edges_out and treats:
 *   true  -- the new edge appeared without the hint = coincidence
 *   false -- the new edge required the hint = causal win
 * The gate short-circuits (returns false, leaves *found_new_edges_out
 * unchanged) on any dispatch-time failure so the classifier can bucket
 * those candidates into the flaky lane.
 *
 * Guarantees:
 *   - Only fires on PC-mode children (the cfactual capture site itself
 *     only fires on the credit_pc(win) arm, but re-check here as a
 *     hard invariant so a stray caller from a CMP-mode context bails).
 *   - in_reexec is set for the duration of the inner dispatch so the
 *     cmp-hint stash push (cmp_hints_stash_consumed) short-circuits
 *     inside the inner generate_syscall_args -- the outer parent's
 *     stash MUST NOT be extended by the replay.
 *   - Same destructive-syscall / AVOID_REEXEC / validate-silent /
 *     slot-bounds gates redqueen_reexec_step uses, for the same
 *     reasons (heap laundering, capability drift, out-of-range slot).
 *   - Shares the redqueen per-window replay cap
 *     (REDQUEEN_REEXEC_WINDOW_CAP): the two consumers cannot each
 *     burn their own cap independently and pin the child on replays.
 *   - Rec snapshot + restore mirrors redqueen_reexec_step exactly so
 *     the parent's dispatched args / retval survive the helper.
 */
					...
					bool *found_new_edges_out)
{
	...
	/* Recursion / mode guards: the outer caller is credit_pc(win) on
	 * a PC-mode child; a CMP-mode or in_reexec entry is a bug in a
	 * future caller and MUST NOT drive an inner dispatch. */
	if (child->in_reexec)
		return false;
	if (child->kcov.mode != KCOV_MODE_PC)
		return false;
	...
	{
		bool inner_found_new = false;
		unsigned long inner_new_cmp = 0;
		unsigned long inner_new_transition = 0;

		ok = dispatch_step(child, entry, &inner_found_new,
				   &inner_new_cmp, &inner_new_transition);
		if (found_new_edges_out != NULL)
			*found_new_edges_out = inner_found_new;
	}
