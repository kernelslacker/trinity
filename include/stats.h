...
#define NR_AF_ALG_PROBE_TEMPLATES	12
	 ...
	 *
	 * RELAXED add-fetch -- diagnostic, not an event log. */
	unsigned long userns_bootstrap_runs;
	unsigned long userns_bootstrap_ran;
	 ...
	 *
	 * Counter shape (mirrors the errno_gradient sibling immediately
	 * above -- a gate scalar plus two non-disjoint subset scalars;
	 * a single bump may bump BOTH subsets when the syscall is both
	 * cold and corpus-absent, which is the common case at fleet
	 * start-of-day):
	 *
	 ...
	 *
	 *  cold_overflow_would_save_cold
	 *      Subset of cold_overflow_would_save: the would-save event
	 *      satisfied the cold side of the disjunction (the syscall
	 *      had not earned a new edge within the cold-skip threshold
	 *      window at the moment the save fired).
	 *
	 *  cold_overflow_would_save_absent
	 *      Subset of cold_overflow_would_save: the would-save event
	 *      satisfied the corpus-absent side of the disjunction (the
	 *      syscall's per-nr ring held no admitted entries at the
	 *      moment the save fired).  This is the headline "the
	 *      single durable lane has not yet admitted this syscall at
	 *      all, yet it is producing CMP novelty" signal.
	 *
