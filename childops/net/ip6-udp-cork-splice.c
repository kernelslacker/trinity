 ...
 *
 * IPv6-only: the v4 __ip_append_data() path has the same shape with its
 * own header math (P1 = mtu - 20 - 8) and would be a separate case.
 */
...
struct ip6_udp_cork_splice_ctx {
...
/*
 * RTM_NEWADDR ipv6 /128 on lo carrying ::1.  Modelled on lo_add_addr()
 * in childops/misc/netns-teardown-churn.c (there is no shared
 * rtnl_add_addr helper).  A fresh netns already brings ::1 up on lo
 * once the interface is UP, so EEXIST here is benign coverage and the
 * caller ignores the return value.
 */
static int lo_add_v6_loopback(struct nl_ctx *ctx, int ifindex)
{
	unsigned char buf[RTNL_BUF_BYTES];
	struct nlmsghdr *nlh;
...
{
	struct ip6_udp_cork_splice_ctx *cctx = (struct ip6_udp_cork_splice_ctx *)arg;
	struct childdata *child = cctx->child;
	struct nl_ctx ctx = { .fd = -1 };
	struct nl_open_opts opts = {
		.proto        = NETLINK_ROUTE,
		.recv_timeo_s = 1,
		.caller_op    = CHILD_OP_IP6_UDP_CORK_SPLICE,
	};
	unsigned char *region = MAP_FAILED;
	struct sockaddr_in6 sin_rx;
	socklen_t slen;
	int tx = -1, rx = -1;
	int lo_idx;
	...
	(void)lo_add_v6_loopback(&ctx, lo_idx);

	rx = socket(AF_INET6, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_UDP);
	direct_calls++;
	if (rx < 0)
		goto out;
	memset(&sin_rx, 0, sizeof(sin_rx));
	sin_rx.sin6_family = AF_INET6;
	sin_rx.sin6_port   = 0;
	sin_rx.sin6_addr   = in6addr_loopback;
	...
	tx = socket(AF_INET6, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_UDP);
	direct_calls++;
	if (tx < 0)
		goto out;
	/* Turn off PMTUD so the kernel does not cache a different PMTU
	 * for ::1 or emit ICMPv6 PTB back at us.  DF still gets set on
	 * the wire but on lo that is moot. */
	(void)setsockopt(tx, IPPROTO_IPV6, IPV6_MTU_DISCOVER,
			 &one, sizeof(one));
	direct_calls++;
...
{
	struct ip6_udp_cork_splice_ctx cctx = { .child = child };
	int rc;
	...
	rc = userns_run_in_ns(CLONE_NEWNET, ip6_udp_cork_splice_in_ns, &cctx);
	if (rc == -EPERM) {
		ns_unsupported = true;
		if (valid_op)
