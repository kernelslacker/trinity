 ...
 * IPv4 UDP corked send path through __ip_append_data()'s continuation-skb
 * length accounting at the exact MTU boundary.  A corked datagram that
 * fills the path MTU exactly (p1 = mtu - iphdr - udphdr), followed by a
 * one-byte flush (p2), drives the second append down the continuation
 * branch, where the copy length (datalen minus the continuation gap) is
 ...
 * __ip_append_data()'s continuation-skb length accounting via ordinary
 ...
 *     (mtu - maxfraglen) bytes that exercises the length computation.
 *
...
static int ip4_udp_cork_splice_in_ns(void *arg)
{
	 ...
	 * __ip_append_data then computes copy = datalen - contgap.
