/*
 * seg6_end_dx4_rx - SRv6 End.DX4 crafted-RX fuzz.
 *
 * Targets the seg6_local input path input_action_end_dx4() /
 * input_action_end_dx4_finish() in net/ipv6/seg6_local.c.  The decap
 * hands the freshly-uncovered inner IPv4 packet to ip_route_input()
 * and then dst_input() without running ip_rcv_core() first, so the
 * skb still carries the outer frame's IP6CB overlay in skb->cb[].
 * Any downstream code that reads the IPv4-side IPCB (options offset
 * in particular) is looking at stale bytes -- __ip_options_echo()
 * reads a bogus option offset from that overlay and copies past its
 * saved-options allocation.  On a KASAN build this manifests as a
 * slab-out-of-bounds write; no extra oracle infra required.
 *
 ...
 *   3. Open AF_PACKET SOCK_RAW bound to lo with sll_protocol=ETH_P_IPV6
 ...
 *          "still routing" vs "at final SID" paths -- End.DX4 fires on
 *          the final SID)
 ...
 *          the ICMP reply path
 *
 * No VRF netdevice needed: End.DX4 takes a raw IPv4 next-hop (NH4)
 * rather than a VRF routing-table lookup, so the setup is simpler
 * than the sibling End.DT4 op.
 *
 ...
 *
 * Latches: ns_unsupported_seg6_end_dx4_rx master gate on
...
#endif
#ifndef SEG6_LOCAL_NH4
#define SEG6_LOCAL_NH4			4
#endif
#ifndef SEG6_LOCAL_ACTION_END_DX4
 ...
 */
static bool ns_unsupported_seg6_end_dx4_rx;

/*
 * SID prefix for the End.DX4 route.  fd00::/8 is ULA -- private,
 ...
 * pointing at lo, with lwtunnel encap SEG6_LOCAL, action End.DX4,
 * nh4=10.0.0.1.  Returns 0 on accept, negated errno on rejection,
 ...
 * DX4 fires when segments_left == 0 (already at the final SID), so
			       ...
			       uint8_t nseg, uint8_t segs_left, uint8_t ihl)
{
	 ...
	 * segments_left == 0 -- End.DX4 fires on that). */
...
struct sed_iter_ctx {
	 ...
	 * childop_direct_syscalls_add() at seg6_end_dx4_rx_in_ns() exit. */
...
static int sed_open_ctx(struct sed_iter_ctx *ctx)
{
	struct nl_open_opts opts = {
		.proto        = NETLINK_ROUTE,
		.recv_timeo_s = 1,
		.caller_op    = CHILD_OP_SEG6_END_DX4_RX,
	};
 ...
 * input_action_end_dx4() runs on delivery.  Each iteration rerolls
...
static void sed_burst(struct sed_iter_ctx *ctx)
{
	...
	iters = BUDGETED(CHILD_OP_SEG6_END_DX4_RX,
	...
	for (i = 0; i < iters; i++) {
		...
		/* segments_left is drawn independently: 0 fires End.DX4;
 ...
 * leaking the raw fd or the seg6local route.  Netns destruction on
 ...
 * socket, per-net seg6 sysctls, and seg6local route left behind are
 * reaped along with the namespace.  Return value is ignored by the
 * helper.
 */
static int seg6_end_dx4_rx_in_ns(void *arg)
{
...
bool seg6_end_dx4_rx(struct childdata *child)
{
	...
	if (ns_unsupported_seg6_end_dx4_rx)
		return true;

	rc = userns_run_in_ns(CLONE_NEWNET, seg6_end_dx4_rx_in_ns, &cctx);
	if (rc == -EPERM) {
		ns_unsupported_seg6_end_dx4_rx = true;
		if (valid_op)
