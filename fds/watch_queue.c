...
static int get_rand_watch_queue_fd(void)
{
	 ...
	 * obj->watch_queueobj.fd deref, mirroring the wireup at 15b6257b8206
	 ...
	 * the watch_queue fd routed into ioctl/read via the fd_provider .get callback (sibling of the keyctl wireup at 5ef98298f6ad),
	...
	for (int i = 0; i < 1000; i++) {
		...
		if ((uintptr_t)obj < 0x10000UL ||
			...
			outputerr("get_rand_watch_queue_fd: bogus obj %p in "
