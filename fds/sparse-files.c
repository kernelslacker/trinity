...
struct object *get_rand_sparse_file_obj(void)
{
	 ...
	 * dereferences obj->sparsefileobj, mirroring the wireup at
	 * 15b6257b8206 (fds/sockets.c get_rand_socketinfo) and
	 * 5ef98298f6ad (syscalls/keyctl.c KEYCTL_WATCH_KEY).  Same
	 * OBJ_GLOBAL lockless-reader UAF window the framework commit
	 * a7fdbb97830c spelled out: between the lockless slot pick and
	 * the consumer's read of the sparse fd / size handed to
	 * lseek(SEEK_DATA|SEEK_HOLE), the parent can destroy the obj,
	 * free_shared_obj() returns the chunk to the shared-heap
