...
static bool json_emit_syscall(const struct syscalltable *table, unsigned int i)
{
	 ...
	 * while attempted_calls is syscall_rt(entry)->attempted (every
	 * dispatched invocation regardless of whether the kcov bracket ran,
	 * so it also counts EXTRA_FORK / validator-rejected / dry-run paths).
	 *
	...
	printf(",\"attempted_calls\":%u,\"kcov_calls\":%lu,\"successes\":%u,\"failures\":%u,\"extrafork_calls\":%lu,\"errnos\":{",
		rt->attempted, kcov_calls, rt->successes,
		rt->failures, extrafork_calls);
	for (j = 0; j <= NR_ERRNOS; j++) {
		if (rt->errnos[j] == 0)
			continue;
		if (!first_errno)
			putchar(',');
		printf("\"%u\":%u", j, rt->errnos[j]);
		first_errno = false;
	}
	fputs("}}", stdout);
	return true;
	...
	if (biarch == true) {
		for_each_32bit_syscall(i) {
			if (syscalls_32bit[i].entry == NULL ||
			    syscall_rt(syscalls_32bit[i].entry)->attempted == 0)
				continue;
			if (!first)
				putchar(',');
		...
		for_each_64bit_syscall(i) {
			if (syscalls_64bit[i].entry == NULL ||
			    syscall_rt(syscalls_64bit[i].entry)->attempted == 0)
				continue;
			if (!first)
				putchar(',');
		...
		for_each_syscall(i) {
			if (syscalls[i].entry == NULL ||
			    syscall_rt(syscalls[i].entry)->attempted == 0)
				continue;
			if (!first)
				putchar(',');
