...
struct rtnl_ack_oracle_stats {
	 ...
	 * The oracle sets NLM_F_ACK on the sampled message in the fuzz
	 * buffer, lets the real sendmsg path send it, then drains the
	 * NLMSG_ERROR reply via MSG_DONTWAIT recv and classifies
	 * err->error into one of these buckets.
	 */
