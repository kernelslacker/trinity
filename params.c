...
static const struct option_help option_descs[] = {
	...
	{ "stats",		 0,  "show errno distribution per syscall before exiting; also writes a per-window stats-timeseries-<epoch>.jsonl in the launch CWD ({\"t\":op_count,\"edges_total\":N,\"edges_gained_this_window\":D,\"per_syscall\":[{\"nr\":N,\"edges\":E,\"edges_gained\":D,\"kcov_calls\":K,\"attempted_calls\":A},...]}) on every print_stats() tick.  Cumulative edges_total / edges are kept; edges_gained_this_window / edges_gained are the per-window deltas (first window's delta = the level) so plateaus are legible without differencing consecutive lines" },
