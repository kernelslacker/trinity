...
/* Inject probability denominator.  Matches the design note's "meaningful
 * share without crowding out random exploration" target -- roughly the
 * same per-call injection rate as the cmp_hints case in
		       ...
		       unsigned long *out)
{
	...
	/* 1-in-PROP_RING_INJECT_MOD gate is the only injection knob in
	 * Phase 1.  Sites that want propagation just call this without
	 * worrying about probability shaping. */
