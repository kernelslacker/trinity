...
struct syscallentry syscall_setitimer = {
	/* setitimer interferes with alarm() (AVOID_SYSCALL).
	 * sanitise_setitimer is re-exec clean (avoid_shared_buffer_out +
	 * zmalloc_tracked snap only, no global side effects): keep
