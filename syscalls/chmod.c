 ...
 * sanitisers (utime, utimensat, xattr-thrash, flock-thrash, ...) touch;
 * cross-process contention concentrates on the same per-inode i_rwsem /
 * notify_change / setattr path.
 */
#define NR_TESTFILES 4

static void sanitise_chmod(struct syscallrecord *rec)
{
	char *path;

	/*
	 * ARG_PATHNAME plumbed a random pathname into rec->a1, but the
	 * random path is most often not a real file at all -- chmod
	 * returns ENOENT at the path walk before ever reaching
	 * chmod_common / notify_change / inode_operations->setattr and
	 * the i_rwsem-guarded per-inode mode-update path.  Classic
	 ...
	 * absolute paths so the subsequent chmod lands on a real
