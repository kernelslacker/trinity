...
static void post_clock_gettime(struct syscallrecord *rec)
{
	 ...
	 * clock_gettime_post_state.  A cookie mismatch means snap does not
	 * point at our struct -- abandon rather than feed wild bytes into
	 * the inner-field deref.  Mirrors recv.c post_recvmsg.
	 */
	if (snap->magic != CLOCK_GETTIME_POST_STATE_MAGIC) {
		outputerr("post_clock_gettime: rejected snap with bad magic 0x%lx "
