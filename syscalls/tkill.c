...
static void sanitise_tkill(struct syscallrecord *rec)
{
	unsigned int draw;

	/*
	 * Bias toward sig==0 (existence-probe, no delivery) and the
	 * ignorable safe set.  tkill has no siginfo path so there is no
