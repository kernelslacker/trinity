...
/*
 * Snapshot of the getxattr input args read by the post oracle, captured
 ...
 */
#define GETXATTR_POST_STATE_MAGIC	0x47585452UL	/* "GXTR" */
						    ...
						    unsigned long pre_a3)
{
	...
	snap->magic    = GETXATTR_POST_STATE_MAGIC;
	snap->value    = rec->a3;
	snap->size     = rec->a4;
...
static void post_getxattr(struct syscallrecord *rec)
{
	 ...
	 * bytes written into the user `value` buffer, which the kernel caps at
	 * the `size` argument (rec->a4) via vfs_getxattr's truncation; failure
