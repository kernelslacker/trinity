...
/*
 * Curated name we plant ahead of the trinity-dispatched lsetxattr.
 * Matches planted_xattr_name in fremovexattr / lremovexattr /
 * lgetxattr / llistxattr so a single round of testfile xattrs is
 * shared across the whole xattr-family precondition surface --
 * lsetxattr's write lands on the same (inode, name) tuple the sibling
 * xattr syscalls read/remove, concentrating cross-process contention
 * on one entry in i_xattrs instead of scattering across the random
 * ARG_XATTR_NAME pool.
 */
