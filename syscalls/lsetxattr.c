...
static void sanitise_lsetxattr(struct syscallrecord *rec)
{
	 ...
	 * random path is most often not a real file at all -- lsetxattr
	 ...
	 * absolute paths so the subsequent lsetxattr lands inside the
	 * real per-inode set path -- the handler dispatch and the
	 * i_xattrs rwsem on a known-existing inode.  lsetxattr is itself
