...
## Files (32 files + 2 internal headers, ~10,562 LOC)

| File | Lines | Role |
|---|---|---|
| range_overlap.c | 802 | `range_overlaps_shared()`/`range_in_tracked_shared()` — mm-syscall sanitiser oracle: bitmap prefilter + exact linear-scan confirm; `CONFIG_GUARD_SHARED` audit path (`range_overlaps_shared_audited`) cross-checks fast vs. slow verdicts |
...
| shared_mem.c | 509 | Core shared-memory allocator: `alloc_shared()`/`alloc_shared_pool()`/`free_shared()`, `alloc_shared_page_aligned()`, region tracking (`shared_regions[]` + overflow tail), `track_shared_region[_tagged]`/`untrack_shared_region` |
| shared_str_heap.c | 396 | Shared string sub-heap: `alloc_shared_str`/`alloc_shared_strdup`/`free_shared_str`, so object names live in shared memory rather than the per-process heap |
| shared_mem_bitmap.c | 325 | 2 MiB-granularity region bitmap plus the size-bucket bitmap: `shared_bitmap_mark`/`unmark`, `tracked_size_mark`/`unmark`, `shared_bitmap_self_check` |
| shared_mem_guard.c | 320 | Per-object `CONFIG_GUARD_SHARED` guard-page mode: `guard_pages_alloc`, span derivation, scope classification and naming, guarded-object census |
| shared_freelist.c | 283 | Size-bucket freelist: `shared_freelist_pop`/`push` with token bounds-checking and CAS slot claim/release |
| corrupt_ptr.c | 660 | `looks_like_corrupted_ptr()` heuristic (NULL-ish/pid-shaped/kernel-VA/misaligned bands) plus per-child attribution rings (by syscall, by callsite PC, by rejection site) for the cluster-1/2/3 SIGSEGV triage |
| locks.c | 505 | `lock_t` primitive: `trylock`/`lock`/`unlock`, dead-owner reaping via (pid, start_time) fingerprint, `check_all_locks()` periodic sweep across shm/children/cmp_hints/minicorpus/chain_corpus lock families, reserved-bit scribble recovery |
| debug.c | 785 | Backtraces, `BUG()`/panic path, child crash dump (`dump_child_bug`, `dump_child_fault_beacon`, `dump_childdata`), list-corruption assertions, `debugf`/`syslogf` |
| isolation.c | 413 | Parent-side one-shot namespace provisioning: `unshare(CLONE_NEWNET\|CLONE_NEWNS)`, hand-rolled netlink RTM_NEWLINK/RTM_NEWADDR to bring up lo+loopback addrs, MS_PRIVATE remount; publishes `net_ready`/`mnt_ready` latches to shm |
...
| utils.c | 287 | Misc: `sizeunit()`, `kill_pid()`, `sanitize_inherited_fds()`, `get_num_fds()`, `cached_online_cpus()` |
| proc-status.c | 293 | `/proc/self/status` seq_file-safe reader (loops read() to EOF) plus field parsers (uint, uid/gid quad, hex sigmask, ns-last-uint) |
| rlimit-safe.c | 246 | Per-resource "safe" (cur,max) pair dictionaries for fuzzing the rlimit family without self-DoS (RLIMIT_NICE encoding, RTPRIO range, etc.) |
| shm.c | 513 | `create_shm()`/`init_shm()` orchestrator: shm sizing, sentinel stamping (netns_fd=-1, scratch_block=-1), phased init (debug/start-time, self-exe snapshot, strategy-rotation state, children[] + fd/stats ring allocation, mprotect(PROT_READ) lockdown, subsystem singleton bring-up) |
...
| uid.c | 153 | `drop_privs()`, `init_uids()`, `check_uid()` — privilege-drop-to-nobody spine |
| writer-watch.c | 140 | `--writer-watch=<addr>`: arms a hardware WRITE breakpoint (perf_event_open) per child for pinpointing a wild writer's exact RIP |
...
| output.c | 108 | `output()`/`outputerr()`/`outputstd()` verbosity-gated logging, `should_route_to_stdout()` (stdout reserved for `--stats-json`) |
| persist-envelope.c | 310 | Shared warm-start envelope: kernel-identity stamping, generation counter, `persist_envelope_validate()` and the on-disk generation manifest read/write shared by every persistence format |
| self_cgroup_setup.c | 561 | cgroup v2 layout: `setup_split()`/`setup_single()` parent/children sub-cgroup construction, memory.high/max caps, OOM-group isolation, `self_cgroup_setup`/`self_cgroup_cleanup` |
| self_cgroup_events.c | 280 | memory.high/max event polling: `events_setup`/`events_cleanup`, `self_cgroup_events_fd`/`self_cgroup_events_check`, `self_cgroup_drop_fds_in_child` |
| self_cgroup_paths.c | 209 | Size-arg parsing/validation (`parse_size_arg`, `validate_cgroup_size_arg`, `mem_total_bytes`) and cgroup path/file IO (`read_self_cg_path`, `write_cg_file`) |
| self_cgroup_delegation.c | 141 | Delegation/capability probing: `already_capped`, `compute_parent_high`, `enable_memory_subtree`, `scope_can_delegate` |
| self_cgroup_fork.c | 114 | `self_cgroup_fork_into_workload()` — clone-into-cgroup of the children sub-cgroup |
| self-cgroup-internal.h | 116 | Declarations private to the `self_cgroup_*.c` cluster |
...
6. **cgroup parent/children split (self_cgroup_setup.c)** — `trinity-<pid>/parent/` (generous `memory.high`, `oom.group=0`) is kept separate from `trinity-<pid>/children/` (`oom.group=1`, capped `memory.max`) so an OOM in the fuzz workload kills the whole worker pool atomically without also killing the orchestrating parent process.
...
- `mm/maps-lifecycle.c`, `mm/maps-initial.c`, `mm/image-segments.c`, `rand/random-address.c` — consume `range_overlaps_shared()`, `range_in_tracked_shared()`, `is_in_glibc_heap()` when picking or validating candidate addresses for fuzzed syscall args.
...
- `cmp_hints/persist.c`, `persist/minicorpus-file.c`, `kcov/persist.c`, `random_syscall/chain-persist.c` — call `persist_sweep_stale_tmp()` at the top of their warm-start load paths, and use `lock_t`/`trylock`/`unlock` for their pool/ring locking; `check_all_locks()` in locks.c walks their lock families by name (cmp_hints pools, minicorpus rings, chain_corpus ring).
...
- `child/child-init-isolate.c`, `main/spawn.c`, `main/trinity-warmstart.c`, `fds/cgroup.c` — drive `self_cgroup_setup()`/`self_cgroup_fork_into_workload()`/`self_cgroup_cleanup()` around the fork boundary.
- `health/signals-fault-handler.c` — `child_fault_handler()` calls `guard_pages_classify()` (async-signal-safe, plain array reads only) to identify a guard-page trap, and consults `range_readable_user()` in the post_snapshot.c recovery path for field-scoped struct dereferences.
...
1. **The self_cgroup cluster does five jobs across five files** — size-arg parsing/validation (self_cgroup_paths.c), cgroup v2 filesystem manipulation (mkdir/write/rmdir across parent+children sub-cgroups), delegation/capability probing (`scope_can_delegate`, `already_capped`, self_cgroup_delegation.c), fork-into-workload orchestration (self_cgroup_fork.c), and `memory.events` polling (self_cgroup_events.c). `setup_split()`/`setup_single()` (self_cgroup_setup.c:76-359) remain the split/fallback cgroup-layout logic and the densest part.
